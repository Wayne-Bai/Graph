...
def Visualize(path, kind, dataset, name, num=None,test_batch=None):
    ...
    if kind == 0:
        prefix = 'GraphRNN_RNN_AST_4_128_pred_%d_test_1'%(name)
    else:
        prefix = 'GraphRNN_RNN_AST%s_4_128_pred_%d_test_1'%(kind, name)
    print(prefix)
    format = '.dat'
    ...
    if num == None:
        for c in range(test_batch):
            ...
            dot.render("process/%s/%s.gv" % (path, str(name)+'-'+'test'+'-'+str(c)), view=True)
        ...
        for i in edge_list:
            m, n = i[0], i[1]
            dot.edge(str(m), str(n))
        dot.render("process/%s/%s-test.gv" %(path,name), view=True)
...
if __name__ == '__main__':
    ...
    for i in range(5000, 8050, 100):
     ...
     #Visualize('code-0818', '50-normalize','50-20',1, None, 8)
