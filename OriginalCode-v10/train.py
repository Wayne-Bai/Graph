...
def test_rnn_epoch(epoch, args, rnn, output, node_f_gen=None, edge_f_gen=None, test_batch_size=16):
    ...
    for i in range(max_num_node):
        ...
        if args.if_add_test_mask and i != 0:
            ...
            node_f_test_mask = generate_test_mask(edge_f_pred_long[:, i-1, :, 2], child_node_f, i)
        ...
        x_step[:, :, :args.max_prev_node] = \
            torch.tensor(torch.tensor(node_edge_info[:, :, -3:], dtype=torch.bool).any(2), dtype=torch.uint8).\
