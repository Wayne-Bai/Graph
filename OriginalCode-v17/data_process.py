...
def encode_adj(adj, max_prev_node=10, is_full=False, is_3D=False):
    ...
    if is_3D:
        ...
        adj_output = np.concatenate((np.zeros((1, max_prev_node)), adj_output),
                                    axis=0)  # Dim: N * M # 0-th row of dim N represents node#0
    return adj_output
...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        adj_encoded = encode_adj(adj_copy.copy(),
                                 max_prev_node=self.max_prev_node)  # Dim: N * 40 (40: max_prev_node, denote as M)
        ...
        # print('*****************************************')
        # print('----------------------------------------------')
        edge_f_encoded = encode_adj(raw_edge_f_batch.copy(), max_prev_node=self.max_prev_node,
                                    is_3D=True)  # Dim: N * M * EF
        ...
        if not args.only_use_adj:
            return {'input_node_f': x_batch, 'raw_node_f': raw_node_f_batch, 'edge_f': edge_f_padded_batch,
                    'len': len_batch}
        else:
            adj_encoded_padded_batch = np.zeros((self.n, self.max_prev_node))
            adj_encoded_padded_batch[:(smallN - 1), :M] = adj_encoded[1:,
                                                          :]  # 0-th row of adj_encoded represents node #0, exclude
            return {'input_node_f': x_batch, 'raw_node_f': raw_node_f_batch, 'edge_f': adj_encoded_padded_batch,
                    'len': len_batch}  # adj_encoded.reshape(-1,adj_encoded.shape[1],1)
    ...
    def construct_edge_f(self, edge_dict, node_num_list, pooling=False):
        ...
        for node_i, i_edge_dict in edge_dict.items():
            for node_j, edge_f_dict in i_edge_dict.items():
                ...
                if node_i in node_num_list and node_j in node_num_list and node_i < node_j and list(
                        edge_f_dict.values()) == l2h_edge:
                    edge_f[node_i - offset][node_j - offset] = np.asarray(l2h_edge)
                elif node_i in node_num_list and node_j in node_num_list and node_i > node_j and list(
                        edge_f_dict.values()) == l2h_edge:
