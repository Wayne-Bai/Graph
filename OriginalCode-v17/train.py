...
def train_rnn_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader): # Fetch graphs of one batch_size; e.g. 32 graphs
        ...
        input_node_f_unsorted = input_node_f_unsorted[:, 0:y_len_max+1, :] # Dim: BS * (N+1) * INF
        ...
        new_y_len = [each+1 for each in y_len] # output_y = y_reshape # Dim: SumN * M * 1
        ...
        h = rnn(input_node_f, pack=True, input_len=new_y_len) # Dim: BS * (N+1) * hidden_size_rnn_output
        h = h[:, 1:, :] # ignore the first row
        ...
        #  direction_loss =
            ...
            edge_f_loss = 0
            node_f_loss = my_cross_entropy(node_f_pred_new, output_node_f,if_CE=True,mask_len=y_len) #+ \
        ...
        if epoch % args.epochs_log==0 and batch_idx==0: # only output first batch's statistics
            print('Epoch: {}/{}, train loss: {:.6f}, node_f_loss: {:.6f}, edge_f_loss: {:.6f}, direction_loss:{:.6f}'.format(
                # epoch, args.epochs,loss.data, node_f_loss.data, edge_f_loss.data, args.num_layers, args.hidden_size_rnn))
                epoch, args.epochs,loss.data, args.node_loss_w * node_f_loss.data, args.edge_loss_w * edge_f_loss, args.edge_loss_w * direction_loss.data))
...
def test_rnn_epoch(epoch, args, rnn, output, node_f_gen=None, edge_f_gen=None, test_batch_size=16, test_set=None):
    ...
    if args.if_test_use_groundtruth:
        ...
        for _, test_data in enumerate(test_set):
            ...
            input_node_f = torch.index_select(input_node_f_unsorted, 0, sort_index)
            edge_f = torch.index_select(edge_f_unsorted, 0, sort_index)
            # save test ground truth graphs
            G_test = nx.Graph()
            input_node_f_data = input_node_f.data.float()
            edge_f_data = edge_f.data.float()
            if args.only_use_adj:
                edge_f_data = edge_f_data[..., np.newaxis]
            for ii in range(test_batch_size):
                G_test = nx.Graph()
                node_idx_list = add_from_node_f_matrix(input_node_f_data[ii,:,args.max_prev_node:].cpu().numpy(), G_test)
                add_from_edge_f_matrix(edge_f_data[ii].cpu().numpy(), G_test, node_idx_list)
            ...
            for bs in range(args.test_batch_size):
                # x_step_temp = x_step[bs,0,args.max_prev_node:args.max_node_type_num+args.max_prev_node]
                # gt_temp = input_node_f[bs,0,args.max_prev_node:args.max_node_type_num+args.max_prev_node]
                # if not torch.all(x_step_temp.eq(gt_temp.cuda())): #x_step_temp != gt_temp.cuda():
                #     print(f'Batch {bs} Node #0 pred type {(x_step_temp==1).nonzero().data} gt type {(gt_temp==1).nonzero().data}')
                # if random.randint(0,1) == 1:
                x_step[bs,0,:] = input_node_f[bs,0,:]
            ...
            for i in range(max_num_node):
                ...
                node_f_pred_slice_list = []
                for bs in range(node_f_pred_step.size(0)):
                    node_f_pred_slice = node_f_pred_step[bs:bs+1,...]
                    if not node_f_pred_slice.bool().any(): # all zeros: non-exist node!
                        node_f_pred_slice = Variable(torch.zeros(node_f_pred_slice.size(0),node_f_pred_slice.size(1),node_f_pred_slice.size(2))).cuda()
                        node_f_pred_slice[:,:,-1] = 1
                    elif torch.max(node_f_pred_slice) == 0: # reasonable values may be less than 0
                        node_f_pred_slice[node_f_pred_slice==0] = -float("Inf")
                        node_f_pred_slice = torch.softmax(node_f_pred_slice, dim=-1)
                        node_f_pred_slice = sample_sigmoid(node_f_pred_slice, sample=False, thresh=args.test_thres, sample_time=1)
                    else:
                        node_f_pred_slice = sample_sigmoid(node_f_pred_slice, sample=False, thresh=args.test_thres, sample_time=1)
                    node_f_pred_slice_list.append(node_f_pred_slice)
                node_f_pred_step = torch.cat(node_f_pred_slice_list, dim=0)
                ...
                # Change some values of x_step to groundtruth
                for bs in range(args.test_batch_size):
                    if i+1 < y_len[bs]: # Should use groundtruth
                        x_step_temp = x_step[bs,0,args.max_prev_node:args.max_node_feature_num+args.max_prev_node]
                        gt_temp = input_node_f[bs,i,args.max_prev_node:args.max_node_feature_num+args.max_prev_node]
                        if not torch.all(x_step_temp.eq(gt_temp.cuda())): #x_step_temp != gt_temp.cuda():
                            print(f'Batch {bs} Node #{i} pred type {(x_step_temp==1).nonzero().data} gt type {(gt_temp==1).nonzero().data}')
                        # if random.randint(0,1) == 1:
                        x_step[bs,0,:] = input_node_f[bs,i+1,:]
    ...
    if args.if_test_use_groundtruth:
        return G_pred_list, G_test_list
    else:
        return G_pred_list
