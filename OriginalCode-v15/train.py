    ...
    for batch_idx, data in enumerate(data_loader): # Fetch graphs of one batch_size; e.g. 32 graphs
        ...
        #  direction_loss =
            ...
            for loss_index, loss_w in enumerate(args.node_loss_w_list):
                ...
                node_f_loss_list[loss_index] = args.node_loss_w * loss_w * node_f_loss_list[loss_index]
        ...
        if epoch % args.epochs_log==0 and batch_idx==0: # only output first batch's statistics
            if not log_all_loss:
                ...
                print('Epoch: {}/{}, train loss: {:.6f}, node_f_loss: {:.6f}, [type_loss {:.6f}, index_loss {:.6f}, int_v_loss {:.6f}, float_v_loss {:.6f}, string_loss {:.6f}], direction_loss:{:.6f}'.format(
                    epoch, args.epochs,loss.data, args.node_loss_w * node_f_loss.data, node_f_loss_list[0].data, node_f_loss_list[1].data, node_f_loss_list[2].data, \
                        node_f_loss_list[3].data, node_f_loss_list[4].data, args.edge_loss_w * direction_loss.data))
...
def test_rnn_epoch(epoch, args, rnn, output, node_f_gen=None, edge_f_gen=None, test_batch_size=16, test_set=None):
    ...
    if args.if_test_use_groundtruth:
        ...
        for _, test_data in enumerate(test_set):
            ...
            G_test = nx.Graph()
            for i in range(test_batch_size):
                node_idx_list = add_from_node_f_matrix(input_node_f_unsorted[i], G_test, new_args=args)
            ...
            for i in range(max_num_node):
                ...
                for bs in range(args.test_batch_size):
                    if i < y_len[bs]: # Should use groundtruth
                        ...
                        gt_temp = input_node_f[bs,i,args.max_prev_node:args.max_node_type_num+args.max_prev_node]
                ...
                for j in range(min(args.max_prev_node,i+1)):
                    ...
                    for bs in range(test_batch_size):
                        indicator = torch.argmax(node_f_pred_long[bs, i-j, :args.max_node_type_num])
