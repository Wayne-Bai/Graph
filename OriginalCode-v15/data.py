...
def adjust_node_values(matrix):
    BS, one, F = matrix.size()
    index_end = args.node_index_num
    int_value_pos = args.node_index_num
    float_value_pos = args.node_index_num + 1
    string_value_start = args.node_index_num + args.node_int_and_float_num
    result = torch.zeros(BS, one, F).cuda()
    for bs in range(BS):
        indicator = torch.argmax(matrix[bs,0,:index_end], dim=-1)
        if indicator == 0: #int
            result[bs,0,0] = 1
            result[bs,0,int_value_pos] = torch.round(matrix[bs,0,int_value_pos])
            result[bs,0,string_value_start] = 1
        elif indicator == 1: #float
            result[bs,0,1] = 1
            result[bs,0,float_value_pos] = matrix[bs,0,float_value_pos]
            result[bs,0,string_value_start] = 1
        elif indicator == 2: #string
            result[bs,0,2] = 1
            str_label = torch.argmax(matrix[bs,0,string_value_start:], dim=-1)
            result[bs,0,string_value_start+str_label] = 1
        elif indicator == 3: #no-value
            result[bs,0,3] = 1
            result[bs,0,string_value_start] = 1
        else:
            raise ValueError
...
def add_from_node_f_matrix(matrix, G:nx.Graph, new_args):
    N, NF = matrix.shape
    node_idx, f_dict = [], []
    for node in range(N):
        ...
        if indicator.any() and indicator[-1] == 0:# a node exists
            ...
            node_features = {f'type{feature_idx}':matrix[node,feature_idx] for feature_idx in range(new_args.max_node_type_num)}
            node_features.update({f'value_type{id}':matrix[node,id+new_args.max_node_type_num] for id in range(new_args.node_index_num)})
            node_features.update({'int_value':matrix[node,new_args.node_index_num+new_args.max_node_type_num]})
            node_features.update({'float_value':matrix[node,new_args.node_index_num+new_args.max_node_type_num+1]})
            node_features.update({f'string_value{id}':matrix[node,id+new_args.max_node_type_num+new_args.node_index_num\
                +new_args.node_int_and_float_num] for id in range(new_args.max_node_value_num)})
...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print('-----------------------------')
        # print(raw_node_f_batch.shape)tr
        ...
        raw_node_v_batch = raw_node_v_batch[x_idx, :]
        ...
        if args.not_use_pooling:
            concat_node_f_batch = np.concatenate((adj_encoded, raw_node_f_batch, raw_node_v_batch), axis=1)
        else:
            concat_node_f_batch = np.concatenate((adj_encoded, raw_node_f_batch, raw_node_v_batch, edge_f_pooled_batch), axis=1)
        ...
        # create raw_node_f_v_batch with paddings
        raw_node_f_v_batch = np.zeros((self.n, raw_node_f_batch.shape[1] + raw_node_v_batch.shape[1]))
        raw_node_f_v_batch[:raw_node_f_batch.shape[0]] = np.concatenate((raw_node_f_batch, raw_node_v_batch), axis=1)
        raw_node_f_v_batch[raw_node_f_batch.shape[0]:, raw_node_f_batch.shape[1]-1] = 1 #non-exist nodes
        ...
        if not args.only_use_adj:
            return {'input_node_f':x_batch,'raw_node_f':raw_node_f_v_batch, 'edge_f':edge_f_padded_batch, 'len':len_batch}
        else:
            ...
            return {'input_node_f':x_batch,'raw_node_f':raw_node_f_v_batch, 'edge_f':adj_encoded_padded_batch, 'len':len_batch} #adj_encoded.reshape(-1,adj_encoded.shape[1],1)
    ...
    def construct_raw_node_v(self, node_dict, node_num_list):
        ...
        N = len(node_dict)
        offset = min(node_num_list)
        index_matrix = np.zeros(shape=(N,4)) # 4 means int, float, string, no value
