...
def generate_mask(size, y_len):
    # generate mask according to y_len: y_len records how many values should be True in each row
    b,maxlen = size[0], size[1]
    X_len = torch.LongTensor(y_len)
    idx = torch.arange(maxlen).unsqueeze(0).expand((b,maxlen))
    len_expanded = X_len.unsqueeze(1).expand((b,maxlen))
    mask = idx < len_expanded
    mask = mask.view(-1).float().cuda()
    total_num = torch.sum(torch.FloatTensor(y_len)).cuda()
    return mask, total_num
...
def my_cross_entropy(y_pred, y,has_weight=False, weight_length=1, weight_max=10,if_CE=False,my_weight=None,mask_len=None):
    '''
    ...
    if has_weight:
        ...
        if if_CE:
            b,n,f = y_pred.shape
            y_pred = y_pred.reshape((b*n,f))
            y = y.reshape(b*n)
            #loss = F.cross_entropy(y_pred, y) 
            my_weight = my_weight.cuda() if not my_weight is None else None      
            if mask_len is None:
                loss = F.cross_entropy(y_pred, y, weight=my_weight)
            else:
                mask, total_len = generate_mask((b,n), mask_len)
                loss_list = F.cross_entropy(y_pred, y, weight=my_weight, reduction='none')
                loss = torch.sum(mask * loss_list) / total_len
        else:
            b,n,f = y_pred.shape
            y_pred = y_pred.reshape((b*n,f))
            y = y.reshape((b*n,f))
            ...
            if mask_len is None:
                ...
                mask, total_len = generate_mask((b,n), mask_len)
                loss_list = loss_func(y_pred, y)
                mask_adjust = mask.view(-1,1).expand(-1,f)
                loss = torch.sum(mask_adjust * loss_list, dim=0) / total_len # return 2 losses: one for int and the other for float
    return loss
...
def sample_sigmoid(y, sample, thresh=0.5, sample_time=2, if_sigmoid=False):
    ...
    # do max likelihood based on some threshold
        ...
        if if_sigmoid:
            ...
            # y_thresh = Variable(torch.ones(y.size(0), y.size(1), y.size(2))*thresh).cuda()
            # y_result = torch.gt(y, y_thresh).float()
            # y_sf = torch.softmax(y, dim=2) 
            # idx = y_sf.argmax(dim=2)
            idx = y.argmax(dim=2)
            y_result = Variable(torch.zeros(y.size(0), y.size(1), y.size(2))).cuda()
            # BAD ERROR!! y_result[:,:,idx] = 1
            for bs in range(y.size(0)):
                for n in range(y.size(1)):
                    y_result[bs, n, idx[bs, n]] = 1
    return y_result
